...
/**
 * A {@link ConcurrentHashMap} that uses {@link ReferenceType#SOFT soft} or {@linkplain ReferenceType#WEAK weak} references for both {@code keys} and
 * {@code values}.
 *
 * <p>
 * This class can be used as an alternative to {@code Collections.synchronizedMap(new WeakHashMap<K, Reference<V>>())} in order to support better performance
 * when accessed concurrently. This implementation follows the same design constraints as {@link ConcurrentHashMap} with the exception that {@code null} values
 * and {@code null} keys are supported.
 *
 * <p>
 * <b>NOTE:</b> The use of references means that there is no guarantee that items placed into the map will be subsequently available. The garbage collector may
 * discard references at any time, so it may appear that an unknown thread is silently removing entries.
 *
 * <p>
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    /**
     * Get the hash for a given object, apply an additional hash function to reduce collisions. This implementation uses the same Wang/Jenkins algorithm as
     ...
     * purged as items are added or removed from the Map. This method can be used to force a purge, and is useful when the Map is read frequently but updated
     * less often.
     */
    ...
    protected final class Segment extends ReentrantLock {
        ...
        /**
         * Array of references indexed using the low order bits from the hash. This property should only be set via {@link #setReferences} to ensure that the
         * {@code resizeThreshold} is maintained.
         */
        private volatile Reference<K, V>[] references;

        /**
         * The total number of references contained in this segment. This includes chained references and references that have been garbage collected but not
         * purged.
         */
        ...
        public <T> T doTask(final int hash, final Object key, final Task<T> task) {
            ...
            if (task.hasOption(TaskOption.SKIP_IF_EMPTY) && this.count == 0) { return task.execute(null, null, null); }
            lock();
            try {
        ...
        /**
         * Restructure the underlying data structure when it becomes necessary. This method can increase the size of the references table as well as purge any
            ...
            while (reference != null) {
                if (reference.getHash() == hash) {
                    ...
                    if (entry != null) {
                        ...
                        if (entryKey == key || entryKey.equals(key)) { return reference; }
                    }
                }
                reference = reference.getNext();
            }
    ...
    /**
     * A reference to an {@link Entry} contained in the map. Implementations are usually wrappers around specific Java reference implementations (e.g.,
     * {@link SoftReference}).
     */
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean contains(Object o) {
            if (o != null && o instanceof Map.Entry<?, ?>) {
                ...
                if (other != null) { return ObjectUtils.nullSafeEquals(entry.getValue(), other.getValue()); }
            }
            return false;
        }
    ...
    protected class ReferenceManager {
        ...
        public Reference<K, V> createReference(Entry<K, V> entry, int hash, Reference<K, V> next) {
            if (ConcurrentReferenceHashMap.this.referenceType == ReferenceType.WEAK) { return new WeakEntryReference<K, V>(entry, hash, next, this.queue); }
            return new SoftEntryReference<K, V>(entry, hash, next, this.queue);
        }

        /**
         * Return any reference that has been garbage collected and can be purged from the underlying structure or {@code null} if no references need purging.
         * This method must be thread safe and ideally should not block when returning {@code null}. References should be returned once and only once.
         * 
