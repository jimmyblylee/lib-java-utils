...
public class InstanceFilter<T> {
     ...
     * @param matchIfEmpty the matching result if both the includes and the excludes collections are empty
     */
    ...
    public boolean match(T instance) {
        ...
        if (!includesSet && !excludesSet) { return this.matchIfEmpty; }

        boolean matchIncludes = match(instance, this.includes);
        boolean matchExcludes = match(instance, this.excludes);

        if (!includesSet) { return !matchExcludes; }

        if (!excludesSet) { return matchIncludes; }
