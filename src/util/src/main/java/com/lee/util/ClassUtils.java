...
public abstract class ClassUtils {
    ...
    static {
        ...
        primitiveTypes.addAll(Arrays.asList(
                new Class<?>[] { boolean[].class, byte[].class, char[].class, double[].class, float[].class, int[].class, long[].class, short[].class }));
    ...
    /**
     * Override the thread context ClassLoader with the environment's bean ClassLoader if necessary, i.e. if the bean ClassLoader is not equivalent to the
     * thread context ClassLoader already.
     * 
     ...
     * <p>
     * This is effectively equivalent to the {@code forName} method with the same arguments, with the only difference being the exceptions thrown in case of
    ...
    public static Class<?> getUserClass(Class<?> clazz) {
        if (clazz != null && clazz.getName().contains(CGLIB_CLASS_SEPARATOR)) {
            ...
            if (superclass != null && Object.class != superclass) { return superclass; }
        }
        return clazz;
    }
     ...
     * @return true for the given class is cache-safe in the given context
     */
    ...
    /**
     * Return a descriptive name for the given object's type: usually simply the class name, but component type class name + "[]" for arrays, and an appended
     ...
     * @return true for the given class matches the user-specified type name.
     */
     ...
     * 
     * @param <T> clazz the clazz to analyze
     * @param clazz the clazz to analyze
     * @param paramTypes the parameter types of the method
    ...
    /**
     * Does the given class or one of its superclasses at least have one or more methods with the supplied name (with any argument types)? Includes non-public
     * methods.
     * 
     * @param clazz the clazz to check
     * @param methodName the name of the method
    ...
    /**
     * Given a method, which may come from an interface, and a target class used in the current reflective invocation, find the corresponding target method if
     * there is one. E.g. the method may be {@code IFoo.bar()} and the target class may be {@code DefaultFoo}. In this case, the method may be
     * {@code DefaultFoo.bar()}. This enables attributes on that method to be found.
     * <p>
     * <b>NOTE:</b> In contrast to {@code org.springframework.aop.support.AopUtils#getMostSpecificMethod}, this method does <i>not</i> resolve Java 5 bridge
     * methods automatically. Call {@code org.springframework.core.BridgeMethodResolver#findBridgedMethod} if bridge method resolution is desirable (e.g. for
     * obtaining metadata from the original method definition).
     * <p>
     ...
     * implementation class, implementations of the {@code GroovyObject} methods will be marked as synthetic anyway). Note that, despite being synthetic, bridge
     * methods ({@link Method#isBridge()}) are considered as user-level methods since they are eventually pointing to a user-declared generic method.
     * 
    ...
    /**
     * Check if the given class represents a primitive (i.e. boolean, byte, char, short, int, long, float, or double) or a primitive wrapper (i.e. Boolean,
    ...
    /**
     * Determine if the given type is assignable from the given value, assuming setting by reflection. Considers primitive wrapper classes as assignable to the
    ...
    /**
     * Return a path suitable for use with {@code ClassLoader.getResource} (also suitable for use with {@code Class.getResource} by prepending a slash ('/') to
     * the return value). Built by taking the package of the specified class file, converting all dots ('.') to slashes ('/'), adding a trailing slash if
     ...
     * {@code org.springframework.core.io.ClassPathResource} is usually even more convenient.
     * 
     ...
     * {@code ClassLoader.getResource()}. For it to be fed to {@code Class.getResource} instead, a leading slash would also have to be prepended to the returned
     * value.
     * 
    ...
    public static Set<Class<?>> getAllInterfacesForClassAsSet(Class<?> clazz, ClassLoader classLoader) {
        Assert.notNull(clazz, "Class must not be null");
        if (clazz.isInterface() && isVisible(clazz, classLoader)) { return Collections.<Class<?>>singleton(clazz); }
     ...
     * @return the common ancestor (i.e. common superclass, one interface extending the other), or {@code null} if none found. If any of the given classes is
     *         {@code null}, the other class will be returned.
     * @since 3.2.6
     */
    public static Class<?> determineCommonAncestor(Class<?> clazz1, Class<?> clazz2) {
        if (clazz1 == null) { return clazz2; }
        if (clazz2 == null) { return clazz1; }
        if (clazz1.isAssignableFrom(clazz2)) { return clazz1; }
        if (clazz2.isAssignableFrom(clazz1)) { return clazz2; }
     ...
     * @return true for the given class is visible in the given ClassLoader.
     */
     ...
     * {@code org.springframework.aop.support.AopUtils#isCglibProxy(Object)} 
     * 
     * @param object the object to check
     * @return true for is cglib proxy
     */
     ...
     * 
     * @param clazz the class to check
     * @return true for is cglib proxy class
     */
     ...
     * @return true for is cglib proxy class name
     */
